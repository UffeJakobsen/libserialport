...
static int libusb_query_port(struct libusb20_device *dev, int idx, char** drv_name_str, char** drv_inst_str)
{
	int rc;
	char *j;
	char sbuf[FILENAME_MAX];

	if (drv_name_str == NULL || drv_inst_str == NULL)
		return -1;

	rc = libusb20_dev_kernel_driver_active(dev, idx);
	if (rc >= 0) {
		sbuf[0] = 0;
		libusb20_dev_get_iface_desc(dev, idx, (char*)&sbuf, (uint8_t)sizeof(sbuf)-1);
		if (sbuf[0] != 0) {
...
static int sysctl_query_dev_drv(const char* drv_name_str, const char* drv_inst_str, char** ttyname, int* const ttyport_cnt)
{
	int rc;
	char sbuf[FILENAME_MAX];
	...
	if (ttyname == NULL || ttyport_cnt == NULL) {
		return -1;
	}

	snprintf(sbuf, sizeof(sbuf)-1, "dev.%s.%s.ttyname", drv_name_str, drv_inst_str);
	...
	if (rc == 0) {
		...
		*ttyname = strndup(tbuf, tbuf_len);
		...
		snprintf(sbuf, sizeof(sbuf)-1, "dev.%s.%s.ttyports", drv_name_str, drv_inst_str);
		tbuf_len = sizeof(tbuf)-1;
		rc = sysctlbyname(sbuf, tbuf, &tbuf_len, NULL, 0);
		if (rc == 0) {
...
static int populate_port_struct_from_libusb_desc(struct sp_port* const port, struct libusb20_device *dev)
{
	char tbuf[FILENAME_MAX];

	// populate port structure from libusb description
	struct LIBUSB20_DEVICE_DESC_DECODED* dev_desc = libusb20_dev_get_device_desc(dev);
	if (dev_desc) {
		...
		if (libusb20_dev_req_string_simple_sync(dev, dev_desc->iManufacturer, tbuf, sizeof(tbuf)) == 0) {
			port->usb_manufacturer = strdup(tbuf);
		}
		if (libusb20_dev_req_string_simple_sync(dev, dev_desc->iProduct, tbuf, sizeof(tbuf)) == 0) {
			port->usb_product = strdup(tbuf);
		}
		if (libusb20_dev_req_string_simple_sync(dev, dev_desc->iSerialNumber, tbuf, sizeof(tbuf)) == 0) {
...
SP_PRIV enum sp_return get_port_details(struct sp_port *port)
{
	...
	while (cua_dev_found == 0) {
		...
		for (idx = 0; idx <= UINT8_MAX-1; idx++) {
			...
			if (rc == 0) {
				rc = sysctl_query_dev_drv(drv_name_str, drv_inst_str, &ttyname, &ttyport_cnt);
				if (rc == 0) {
